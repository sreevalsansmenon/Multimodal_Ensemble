...
function t1w_custom(XTrain,YTrain,XTest,YTest,crossfold)

dlXTest = dlarray(XTest,'SSSCB');   % create deep learning array
classes = categories(YTrain);       % retrieve the class information with the type of categorical
numClasses = numel(classes);        % Retrieve numbe rof classes

dlnet1=createLayer_1(XTrain);       % Create deep learning layer
...
for epoch = 1:numEpochs
    ...
    function dlnet=createLayer_1(XTrain)
        layers = [
            image3dInputLayer([91 109 91 3],'Name','fmri_input',"Mean",mean(XTrain,5))

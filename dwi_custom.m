...
function dwi_custom(XTrain,YTrain,XTest,YTest,crossfold)
...
for epoch = 1:numEpochs
    ...
    function dlnet=createLayer_1(XTrain)
        layers = [
            image3dInputLayer([91 109 91 2],'Name','fmri_input',"Mean",mean(XTrain,5))
            convolution3dLayer(3,4,'Padding','same','Name','conv_f1')
            reluLayer('Name','relu_f1')
            maxPooling3dLayer(2,'Stride',2, 'Name','maxpool_f1','Padding','same')
            convolution3dLayer(3,8,'Padding','same','Name','conv_f2')
            reluLayer('Name','relu_f2')
            maxPooling3dLayer(2,'Stride',2, 'Name','maxpool_f2','Padding','same')
